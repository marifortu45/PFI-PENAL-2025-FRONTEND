...
### Esquema Principal (PostgreSQL en RDS)
...
resultado           VARCHAR(20)  -- 'Gol', 'Atajado', 'Afuera'
direccion           VARCHAR(20)  -- 'Izquierda', 'Centro', 'Derecha'
altura              VARCHAR(20)  -- 'Arriba', 'Medio', 'Abajo'
pie_usado           CHAR(1)      -- 'L', 'R'
video_url           TEXT         -- URL en S3
...
# SageMaker
SAGEMAKER_ENDPOINT_NAME=penal-predictor-endpoint
...
```bash
docker build -t penal-frontend .
docker run -p 80:3000 penal-frontend
```
...
test('renders homepage with 4 modules', () => {
  render(<HomePage onNavigate={() => {}} />);
  expect(screen.getByText(/Ver Videos con Posturas/i)).toBeInTheDocument();
  expect(screen.getByText(/Cargar Video Nuevo/i)).toBeInTheDocument();
  expect(screen.getByText(/Predicción de Penal/i)).toBeInTheDocument();
  expect(screen.getByText(/Ver Jugadores/i)).toBeInTheDocument();
});
```
...
### ✅ Implementado
- [x] Sistema de navegación entre vistas
- [x] Lista y búsqueda de jugadores
- [x] Upload de videos con metadatos completos
- [x] Detección de jugadores con YOLOv11
- [x] Sistema de predicción con múltiples modelos
- [x] Reproductor de video con análisis
...
- [ ] Sistema de autenticación (Auth0 / Cognito)

### 🔮 Futuro
- [ ] Dashboard de analytics avanzado
- [ ] Modo multijugador para sesiones de entrenamiento
...
- **Universidad/Organización**: [Tu Institución]
- **Año**: 2025
...
- **GitHub Issues**: [Link a issues del repo]
- **Documentación Completa**: [Link a docs si existe]
- **Demo en Vivo**: [Link a demo si está deployado]

---

<div align="center">

**Desarrollado con ❤️ para mejorar el análisis deportivo**

⚽ **PENAL** - Donde la tecnología encuentra el fútbol
